...
for i=1:numFiles
    ...
    for j=1:length(allMaps)
        ...
        allMaps(j) = allMaps(j).skeletonAnalysis(1, [21 24]); % re-run skeleton analysis on channel 1
    end
    ...
    odorATPmaps = regexp(descriptions,'..2');
    ...
    odorAlonemaps = regexp(descriptions,'..1');
